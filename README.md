...
1. Beating the Accuracy Paradox: Initially, the model hit 99% accuracy but 0% recall because it learned to predict "No Planet" for every star to get a high score. I pivoted to Recall as the primary metric, using SMOTE and custom class weighting to force the AI to prioritize finding the rare $1\%$ of actual planets.
2. Solving the SHAP Dimension Mismatch: Integrating XAI caused a dimension crash between the 3D CNN input and the 2D SHAP explainer. I engineered a custom reshaping wrapper to align the background data, allowing the heatmaps to map perfectly onto physical transit dips.
3. Killing "Ghost Signals": To stop the AI from flagging random sensor glitches, I implemented a Scientific Vetting Layer. By using Phase Folding and a strict 1.5 SNR threshold, exHUMA ensures that every candidate on the leaderboard is a physically repeating transit.
